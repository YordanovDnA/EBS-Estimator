...
// ✅ Unit price fallback for additionals (if your UI doesn't store add.price)
const ADDITIONAL_UNIT_PRICES = {
  socket: 85,
  radiator: 220,
  skip: 320,
  toilet: 160,
...
function calculateQuote(formData) {
  ...
  /* ---------- ADDITIONALS (fixed totals + price fallback) ---------- */
  const additionals = [];
  ...
  if (Array.isArray(formData.additionals)) {
    formData.additionals.forEach((add) => {
      ...
      const unitPrice =
        Number(add.price) || ADDITIONAL_UNIT_PRICES[add.id] || 0;
...
export default function FinalQuoteStep({ formData }) {
  ...
  function renderAdditionalsHtml() {
    if (!quote.additionals?.length) return "";

    const rows = quote.additionals
      .map(
        (a) => `
          <tr>
            <td style="padding:6px 8px; border-bottom:1px solid #2A2A2A;">
              ${escapeHtml(a.name)} (×${a.quantity})
              ${
                a.unitPrice
                  ? `<span style="opacity:0.8;"> — £${Number(
                      a.unitPrice
                    ).toLocaleString()} each</span>`
                  : ""
              }
            </td>
            <td style="padding:6px 8px; border-bottom:1px solid #2A2A2A; text-align:right;">
    ...
    const total = quote.additionals.reduce(
      (s, a) => s + (Number(a.total) || 0),
      0
    );

    return `
      <h3 style="margin:16px 0 8px 0; color:#C8A74A;">Additional Services</h3>
      <table style="width:100%; border-collapse:collapse;">
        <tbody>
          ${rows}
          <tr>
            <td style="padding:8px; font-weight:bold;">Additionals Total</td>
            <td style="padding:8px; font-weight:bold; text-align:right; color:#C8A74A;">
  ...
  function wrapEmailHtml(innerHtml) {
    return `
      <div style="background:#0E0E0E; padding:18px; font-family: Arial, sans-serif; color:#F5F5F5;">
        <div style="max-width:720px; margin:0 auto; border:1px solid rgba(200,167,74,0.25); border-radius:12px; overflow:hidden;">
          <div style="padding:14px 16px; background:linear-gradient(135deg, rgba(200,167,74,0.18), rgba(216,184,92,0.12)); border-bottom:1px solid rgba(200,167,74,0.25);">
            <div style="font-size:14px; letter-spacing:0.08em; color:#C8A74A; font-weight:bold;">
              EXCEPTIONAL BUILDING SERVICES
            </div>
            <div style="margin-top:6px; font-size:12px; opacity:0.9;">
              Automated Estimate Submission
            </div>
          </div>
          ...
          <div style="padding:12px 16px; border-top:1px solid rgba(200,167,74,0.2); font-size:12px; opacity:0.85;">
            <div>All estimates are subject to site survey and final specification.</div>
            <div style="margin-top:6px;">EBS • Bristol • ebs-team.co.uk</div>
          </div>
        </div>
      </div>
    `;
  ...
  function buildInternalEmailHTML() {
    const inner = `
      <h2 style="margin:0 0 10px 0; color:#C8A74A;">New Quote Request</h2>

      <table style="width:100%; border-collapse:collapse; margin-bottom:14px;">
        <tr><td style="padding:6px 0; opacity:0.85;">Name</td><td style="padding:6px 0;"><strong>${escapeHtml(
          customerDetails.fullName
        )}</strong></td></tr>
        <tr><td style="padding:6px 0; opacity:0.85;">Email</td><td style="padding:6px 0;">${escapeHtml(
          customerDetails.email
        )}</td></tr>
        <tr><td style="padding:6px 0; opacity:0.85;">Phone</td><td style="padding:6px 0;">${escapeHtml(
          customerDetails.phone || "Not provided"
        )}</td></tr>
        <tr><td style="padding:6px 0; opacity:0.85;">Address</td><td style="padding:6px 0;">${escapeHtml(
        ...
        <tr><td style="padding:6px 0; opacity:0.85;">Notes</td><td style="padding:6px 0;">${escapeHtml(
          customerDetails.notes || "None"
        )}</td></tr>
      </table>

      <h3 style="margin:16px 0 8px 0; color:#C8A74A;">Project Summary</h3>

      ${quote.services
        .map(
          (s) => `
            <div style="margin:0 0 14px 0; padding-bottom:12px; border-bottom:1px solid #2A2A2A;">
              <div style="display:flex; justify-content:space-between; gap:12px;">
                <div><strong>${escapeHtml(s.name)}</strong></div>
                <div style="color:#C8A74A; font-weight:bold;">£${Number(
                  s.costLow
      ...
      ${renderAdditionalsHtml()}

      <h3 style="margin:16px 0 8px 0; color:#C8A74A;">Total Estimate</h3>
      <div style="padding:12px; border:1px solid rgba(200,167,74,0.25); border-radius:10px; background:rgba(200,167,74,0.08);">
        <div style="font-size:18px; font-weight:bold; color:#C8A74A;">
          £${Number(quote.totalLow).toLocaleString()} – £${Number(
      quote.totalHigh
    ).toLocaleString()}
        </div>
        <div style="margin-top:6px; font-size:12px; opacity:0.9;">
          Duration: ${quote.durationDays} days (${quote.projectWeeks} weeks)
        </div>
      </div>
    `;
  ...
  // ✅ Customer gets full breakdown too (since they already saw it on screen)
  function buildCustomerEmailHTML() {
    const inner = `
      <h2 style="margin:0 0 10px 0; color:#C8A74A;">Your Estimate Summary</h2>
      <p style="margin:0 0 12px 0;">
        Thank you, <strong>${escapeHtml(
          customerDetails.fullName
        )}</strong>. Below is a copy of your estimate and selections.
      </p>

      <div style="padding:12px; border:1px solid rgba(200,167,74,0.25); border-radius:10px; background:rgba(200,167,74,0.08); margin-bottom:14px;">
        <div style="font-size:18px; font-weight:bold; color:#C8A74A;">
          Total: £${Number(quote.totalLow).toLocaleString()} – £${Number(
      quote.totalHigh
    ).toLocaleString()}
        </div>
        <div style="margin-top:6px; font-size:12px; opacity:0.9;">
          Estimated duration: ${quote.durationDays} days (${
      quote.projectWeeks
    } weeks)
        </div>
      </div>

      <h3 style="margin:16px 0 8px 0; color:#C8A74A;">Breakdown</h3>
      ...
      <p style="margin-top:14px; font-size:12px; opacity:0.9;">
        This is an estimate only. Final costs can change following a site survey, specification, and access conditions.
      </p>
      <p style="margin-top:10px;">
        Best regards,<br/>
        <strong>Exceptional Building Services</strong>
      </p>
    `;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("EMAIL ERROR: " + (err?.message || "Unknown error"));
    }
  };
          ...
          {quote.additionals?.length > 0 && (
            <div className="pb-2 border-b border-[#262626]">
              ...
              <div className="ml-3 space-y-0.5">
                ...
                <p className="text-xs text-[#C8A74A] mt-1 font-semibold">
                  Total additionals: £
                  {Number(quote.additionalsTotal || 0).toLocaleString()}
                </p>
              </div>
            </div>
          )}
...
function formatKitchenRoom(room) {
  ...
  if (room.floorTiling != null && room.floorTiling !== 0)
    bullets.push(`Floor tiling: ${room.floorTiling} m²`);
  return bullets;
}

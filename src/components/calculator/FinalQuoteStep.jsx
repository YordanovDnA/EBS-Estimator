...
import { Mail, Calendar, HardHat, CheckCircle2, Home, Building2, Building } from "lucide-react";
...
// Rate tiers
const RATES = {
  economy: { daily: 200, efficiency: 1.00 },
  standard: { daily: 250, efficiency: 0.90 },
...
function generateModuleDetails(formData) {
  ...
  if (formData.kitchen?.areas) {
    ...
    formData.kitchen.areas.forEach(area => {
      const parts = [];
      const sizeLabel = { small: 'Small', medium: 'Medium', large: 'Large' }[area.size] || area.size;
      ...
      if (area.requireElectricalAlterations && area.electrics?.length > 0) {
        parts.push(`electrical: ${area.electrics.map(e => e.replace('_', ' ')).join(', ')}`);
      }
      if (area.requirePlumbingAlterations && area.plumbing?.length > 0) {
        parts.push(`plumbing: ${area.plumbing.map(p => p.replace('_', ' ')).join(', ')}`);
      }
      if (area.splashback) parts.push('splashback');
      if (area.requireFloorTiling && area.floorTilingArea > 0) parts.push(`floor tiles ${area.floorTilingArea} m²`);
  ...
  if (formData.bathroom?.rooms) {
    ...
    formData.bathroom.rooms.forEach(room => {
      const parts = [];
      const sizeLabel = { small: 'Small', medium: 'Medium', large: 'Large' }[room.size] || room.size;
      const layoutLabel = { keep: 'Keep', minor: 'Minor', major: 'Major' }[room.layout];
      
      parts.push(`${room.name} (${sizeLabel}, ${layoutLabel} layout)`);
      
      if (room.fixtures?.length > 0) {
        const fixturesText = room.fixtures.map(f => f.replace('_', ' ')).join(', ');
      ...
      if (room.requireElectricalAlterations && room.electrics?.length > 0) {
        parts.push(`electrics: ${room.electrics.map(e => e.replace('_', ' ')).join(', ')}`);
      }
      
      if (room.requirePlumbingAlterations && room.plumbing?.length > 0) {
        parts.push(`plumbing: ${room.plumbing.map(p => p.replace('_', ' ')).join(', ')}`);
      }
  ...
  if (formData.flooring?.areas) {
    ...
    formData.flooring.areas.forEach(area => {
      ...
      items.push(`${area.name}: ${typeLabel}, ${area.area} m²; ${area.pattern}`);
    });
    details.push({ items });
  } else {
    details.push(null);
  ...
  if (formData.carpentry?.areas) {
    ...
    formData.carpentry.areas.forEach(area => {
      const parts = [];
      if (area.doorCount > 0) parts.push(`${area.doorCount} door${area.doorCount > 1 ? 's' : ''}`);
      if (area.skirtingMetres > 0) parts.push(`${area.skirtingMetres} m skirting`);
      if (area.architraveMetres > 0) parts.push(`${area.architraveMetres} m architrave`);
      if (area.wardrobeMetres > 0) parts.push(`${area.wardrobeMetres} m storage`);
      if (parts.length > 0) {
  ...
  if (formData.painting?.rooms) {
    ...
    formData.painting.rooms.forEach(room => {
      ...
      const parts = [`${room.name} (${typeLabel}): ${room.size}`, surfaces.join('+')];
      ...
      if (room.doors > 0) parts.push(`${room.doors} door${room.doors > 1 ? 's' : ''}`);
      if (room.windows > 0) parts.push(`${room.windows} window${room.windows > 1 ? 's' : ''}`);
      
      if ((room.type === 'stairs' || room.type === 'hall_stairs') && room.spindleCount > 0) {
...
function calculateQuote(formData) {
  ...
  if (formData.kitchen?.areas) {
    ...
    formData.kitchen.areas.forEach(area => {
      ...
      let baseDaysLow = area.size === 'small' ? 3 : area.size === 'large' ? 8 : 5;
      let baseDaysHigh = area.size === 'small' ? 5 : area.size === 'large' ? 12 : 8;
  ...
  if (formData.bathroom?.rooms) {
    ...
    formData.bathroom.rooms.forEach(room => {
      ...
      let baseDaysLow = room.size === 'small' ? 5 : room.size === 'large' ? 12 : 8;
      let baseDaysHigh = room.size === 'small' ? 8 : room.size === 'large' ? 18 : 12;

      const layoutMult = room.layout === 'keep' ? 1.0 : room.layout === 'minor' ? 1.10 : 1.25;
      ...
      const plumbingComplexity = plumbingCount === 0 ? 'light' : plumbingCount <= 2 ? 'moderate' : 'heavy';
    ...
    services.push({ name: 'Bathroom / WC', daysLow, daysHigh, costLow, costHigh });
    totalDaysLow += daysLow;
    totalDaysHigh += daysHigh;
    totalCostLow += costLow;
    totalCostHigh += costHigh;
  ...
  if (formData.flooring?.areas) {
    ...
    const costLow = Math.round(totalFlooringCostLow * efficiency * propertyMultiplier / 5) * 5;
    const costHigh = Math.round(totalFlooringCostHigh * efficiency * propertyMultiplier / 5) * 5;

    services.push({ name: 'Flooring & Tiling', daysLow, daysHigh, costLow, costHigh });
    totalDaysLow += daysLow;
    totalDaysHigh += daysHigh;
    totalCostLow += costLow;
    totalCostHigh += costHigh;
  ...
  if (formData.carpentry?.areas) {
    ...
    services.push({ name: 'Carpentry & Joinery', daysLow, daysHigh, costLow, costHigh });
    totalDaysLow += daysLow;
    totalDaysHigh += daysHigh;
    totalCostLow += costLow;
    totalCostHigh += costHigh;
  ...
  if (formData.painting?.rooms) {
    ...
    formData.painting.rooms.forEach(room => {
      ...
      if (room.type === 'stairs' || room.type === 'hall_stairs') {
        if (room.staircaseHeight === 'double') roomDays += 0.40;
        roomDays += 0.02 * (room.spindleCount || 0);
        if (room.handrailsStringers) roomDays += 0.10;
      }
    ...
    services.push({ name: 'Painting & Decorating', daysLow, daysHigh, costLow, costHigh });
    totalDaysLow += daysLow;
    totalDaysHigh += daysHigh;
    totalCostLow += costLow;
    totalCostHigh += costHigh;
  ...
  if (formData.plastering?.areas) {
    ...
    formData.plastering.areas.forEach(area => {
      ...
      if (area.workType === 'patch') {
        ...
        baseDays = ((area.area || 0) / 27.5) * 1.40;
      }
    ...
    services.push({ name: 'Plastering & Patching', daysLow, daysHigh, costLow, costHigh });
    totalDaysLow += daysLow;
    totalDaysHigh += daysHigh;
    totalCostLow += costLow;
    totalCostHigh += costHigh;
  ...
  if (formData.materials && formData.materials.includeInTotal) {
    ...
    if (formData.materials.mode === 'ballpark' && formData.materials.selected) {
      const BALLPARK_PRICES = {
        cabinets: { basic: [1000, 2000], standard: [2000, 3500], premium: [3500, 6000] },
        worktops: { basic: [40, 80], standard: [80, 140], premium: [220, 380] },
        appliances: { basic: [900, 1400], standard: [1400, 2200], premium: [2200, 3500] },
        sink: { basic: [120, 250], standard: [250, 450], premium: [450, 800] },
        radiator: { basic: [90, 150], standard: [150, 250], premium: [250, 450] },
        bathroom_suite: { basic: [500, 900], standard: [900, 1600], premium: [1600, 3000] },
        flooring: { basic: [12, 25], standard: [20, 40], premium: [30, 60] },
        kitchen_tiles: { basic: [18, 28], standard: [28, 45], premium: [45, 75] },
        bathroom_tiles: { basic: [18, 28], standard: [28, 45], premium: [45, 75] },
        paint: { basic: [25, 40], standard: [40, 70], premium: [70, 120] },
        lighting: { basic: [20, 40], standard: [40, 80], premium: [80, 150] },
        bathroom_lighting: { basic: [20, 40], standard: [40, 80], premium: [80, 150] },
        kitchen_hardware: { basic: [40, 80], standard: [80, 150], premium: [150, 280] },
        bathroom_hardware: { basic: [40, 80], standard: [80, 150], premium: [150, 280] },
        ...
        joinery_hardware: { basic: [30, 50], standard: [50, 90], premium: [90, 150] },
        ...
        electrical: { basic: [50, 80], standard: [80, 140], premium: [140, 250] },
        common_hardware: { basic: [40, 70], standard: [70, 120], premium: [120, 200] }
      };

      Object.entries(formData.materials.selected).forEach(([itemId, config]) => {
        ...
        if (prices && prices[config.quality]) {
          ...
          const qualityLabel = config.quality.charAt(0).toUpperCase() + config.quality.slice(1);
    ...
    } else if (formData.materials.mode === 'custom' && formData.materials.custom) {
      const total = formData.materials.custom.reduce((sum, item) => sum + (item.amount || 0), 0);
...
export default function FinalQuoteStep({ formData }) {
  ...
  const validateForm = () => {
    ...
    if (!customerDetails.addressLine1.trim()) errors.addressLine1 = "Address is required";
    if (!customerDetails.city.trim()) errors.city = "City/Town is required";
    if (!customerDetails.postcode.trim()) errors.postcode = "Postcode is required";
  ...
  return (
    <div className="theme-dark quote-screen">
      ...
      <Card className="bg-gradient-to-br from-[#C8A74A]/20 to-[#D8B85C]/20 border-[#C8A74A]/50 p-4 sm:p-5 mb-2 text-center">
        <p className="text-[#C8A74A] text-xs font-medium mb-1">ESTIMATED TOTAL COST</p>
        <div className="text-2xl sm:text-3xl font-bold text-[#C8A74A]">
          £{quote.totalLow.toLocaleString()} - £{quote.totalHigh.toLocaleString()}
        </div>
      </Card>
      ...
      <div className="p-2 sm:p-3 mb-4 bg-blue-500/10 border border-blue-500/30 rounded-lg text-center">
        <p className="text-xs text-[#F5F5F5]">
          💡 This is an estimated quote. Final costs may vary based on specific requirements, site conditions, and material selections. All estimates subject to site survey.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-4">
        ...
        <Card className="bg-[#151515] border-[#262626] p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-[#F5F5F5]">Property Type</p>
      ...
      <Card className="bg-[#151515] border-[#262626] p-3 sm:p-4 mb-4">
        <h3 className="text-sm font-semibold text-[#C8A74A] mb-3 module-title">Cost Breakdown</h3>
        <div className="space-y-2 cost-breakdown" style={{ lineHeight: 1.3 }}>
          {quote.services.map((service, idx) => (
            <div key={idx} className="pb-2 border-b border-[#262626] last:border-0">
              <div className="flex justify-between items-baseline mb-1">
                <span className="text-sm text-[#F5F5F5] font-medium">{service.name}</span>
                <span className="text-sm font-semibold text-[#C8A74A]">
                  £{service.costLow.toLocaleString()} - £{service.costHigh.toLocaleString()}
                </span>
              </div>
              {moduleDetails[idx] && moduleDetails[idx].items && moduleDetails[idx].items.length > 0 && (
                <div className="ml-3 space-y-0.5 mt-1">
                  {moduleDetails[idx].items.map((detail, i) => (
                    <p key={i} className="text-xs text-[#F5F5F5] bullet">• {detail}</p>
                  ))}
                </div>
              )}
            </div>
          ...
          {quote.additionals.length > 0 && (
            <div className="pb-2 border-b border-[#262626]">
              <p className="text-sm text-[#F5F5F5] font-medium mb-1">Additional Services</p>
              <div className="ml-3 space-y-0.5">
                {quote.additionals.map((add, idx) => (
                  <p key={idx} className="text-xs text-[#F5F5F5] bullet">
                    • {add.name} (×{add.quantity}) — £{add.total.toLocaleString()}
                  </p>
                ))}
              </div>
            </div>
          ...
          {quote.materialsIncluded && quote.materialsMid > 0 && (
            <div className="pb-2 border-b border-[#262626]">
              <div className="flex justify-between items-baseline mb-1">
                <span className="text-sm text-[#F5F5F5] font-medium">Materials</span>
                <span className="text-sm font-semibold text-[#C8A74A]">
              ...
              {quote.materialsList.length > 0 && (
                <div className="ml-3 space-y-0.5 mt-1">
                  {quote.materialsList.slice(0, 5).map((item, i) => (
                    <p key={i} className="text-xs text-[#F5F5F5] bullet">• {item}</p>
                  ))}
                  {quote.materialsList.length > 5 && (
                    <p className="text-xs text-[#F5F5F5] bullet">• ...and {quote.materialsList.length - 5} more</p>
                  )}
                </div>
              )}
            </div>
          ...
          {quote.designManagement > 0 && (
            <div className="pb-2">
              <div className="flex justify-between items-baseline mb-1">
                <span className="text-sm text-[#F5F5F5] font-medium">Design & Management</span>
                <span className="text-sm font-semibold text-[#C8A74A]">
              ...
              <div className="ml-3">
                <p className="text-xs text-[#F5F5F5] bullet">
                  • {formData.designManagement === 'procurement' ? 'Procurement (7%)' :
                     formData.designManagement === 'management_procurement' ? 'Management & Procurement (11%)' :
                     formData.designManagement === 'full' ? 'Design, Management & Procurement (12%)' :
                     'Design & Management'}
                </p>
              </div>
            </div>
      ...
      <Card className="bg-gradient-to-br from-[#151515] to-[#0E0E0E] border-[#C8A74A]/30 p-3 sm:p-4 mb-4">
        <h3 className="text-sm font-semibold text-[#C8A74A] mb-1">Your Details</h3>
        <p className="text-xs text-[#F5F5F5] mb-3">
          Enter your details to receive a comprehensive breakdown and schedule a consultation
        </p>
        ...
        {submitted ? (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-[#F5F5F5]">
              Your detailed quote request has been submitted successfully. We'll be in touch soon.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              <Label htmlFor="fullName" className="text-[#F5F5F5] mb-1 block text-xs">
              ...
              <Input
                ...
                onChange={(e) => updateCustomerDetail('fullName', e.target.value)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <Label htmlFor="email" className="text-[#F5F5F5] mb-1 block text-xs">
              ...
              <div>
                <Label htmlFor="phone" className="text-[#F5F5F5] mb-1 block text-xs">Phone <span className="text-[#F5F5F5]">(Optional)</span></Label>
                <Input
                  id="phone"
                  type="tel"
                  inputMode="tel"
            ...
            <div>
              <Label htmlFor="addressLine1" className="text-[#F5F5F5] mb-1 block text-xs">
              ...
              <Input
                ...
                onChange={(e) => updateCustomerDetail('addressLine1', e.target.value)}
                className={`bg-[#0E0E0E] border-[#262626] text-[#F5F5F5] h-9 text-sm focus:border-[#C8A74A] focus:outline-none focus:ring-1 focus:ring-[#C8A74A] ${
            ...
            <div>
              <Label htmlFor="addressLine2" className="text-[#F5F5F5] mb-1 block text-xs">
                Address Line 2 <span className="text-[#F5F5F5]">(Optional)</span>
              </Label>
              <Input
                id="addressLine2"
                type="text"
                ...
                onChange={(e) => updateCustomerDetail('addressLine2', e.target.value)}
                className="bg-[#0E0E0E] border-[#262626] text-[#F5F5F5] h-9 text-sm focus:border-[#C8A74A] focus:outline-none focus:ring-1 focus:ring-[#C8A74A]"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <Label htmlFor="city" className="text-[#F5F5F5] mb-1 block text-xs">
              ...
              <div>
                <Label htmlFor="postcode" className="text-[#F5F5F5] mb-1 block text-xs">
                ...
                <Input
                  ...
                  onChange={(e) => updateCustomerDetail('postcode', e.target.value.toUpperCase())}
                  className={`bg-[#0E0E0E] border-[#262626] text-[#F5F5F5] h-9 text-sm focus:border-[#C8A74A] focus:outline-none focus:ring-1 focus:ring-[#C8A74A] ${
                    validationErrors.postcode ? 'border-red-500' : ''
                  }`}
                />
                {validationErrors.postcode && (
                  <p className="text-red-400 text-xs mt-1">{validationErrors.postcode}</p>
                )}
              </div>
            </div>

            <div>
              <Label htmlFor="notes" className="text-[#F5F5F5] mb-1 block text-xs">
                Additional Notes for Your Quote <span className="text-[#F5F5F5]">(Optional)</span>
              </Label>
              <textarea
                id="notes"
                rows={3}
